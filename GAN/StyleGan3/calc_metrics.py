...
def calc_metrics(ctx, network_pkl, metrics, data, mirror, gpus, verbose):
    ...
    # Load network.
    if not GAN.StyleGan3.dnnlib.util.is_url(network_pkl, allow_file_urls=True) and not os.path.isfile(network_pkl):
        ctx.fail('--network must point to a file or URL')
    if args.verbose:
        print(f'Loading network from "{network_pkl}"...')
    with GAN.StyleGan3.dnnlib.util.open_url(network_pkl, verbose=args.verbose) as f:
