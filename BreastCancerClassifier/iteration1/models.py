...
from BreastCancerClassifier.iteration1.constants import VIEWS, VIEWANGLES
...
class modifiedDensenet169(nn.Module):
    ...
    def forward(self, x1):
        ...
        h2 = self.all_views_gaussian_noise_layer.single_add_gaussian_noise(x1)

        result2 = self.model_1(h2)

        result2 = self.all_views_avg_pool.single_avg_pool(result2)

        result2 = self.output_layer(result2)

        return result1, result2, torch.cat((result1,result2),dim=0), torch.stack((result1,result2),dim=0)
